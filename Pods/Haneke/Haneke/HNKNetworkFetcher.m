...
- (void)fetchImageWithSuccess:(void (^)(UIImage *image))successBlock failure:(void (^)(NSError *error))failureBlock;
{
    ...
    _dataTask = [self.URLSession dataTaskWithURL:_URL completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        // Try creating SVG image from data
        // The size were not defined in SVG xml and UIImage+SVG plugin cannot work without it so I placed some static size after looking into SVG xml viewpoint
        if (!image)
            image = [UIImage imageWithSVGData:data targetSize:CGSizeMake(112, 126) fillColor:[UIColor blackColor] cache:NO];
